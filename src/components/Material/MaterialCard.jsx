...
function iconByType(tipo) {
  switch (tipo) {
    case 'pdf':    return <FileText className="h-7 w-7 text-rose-500" />;
    case 'slide':  return <Presentation className="h-7 w-7 text-amber-500" />;
    case 'video':  return <PlayCircle className="h-7 w-7 text-violet-500" />;
    case 'repo':   return <Github className="h-7 w-7 text-gray-800" />;
    default:       return <LinkIcon className="h-7 w-7 text-primary" />;
  }
}

export default function MaterialCard({ item, isAdmin }) {
  return (
    <div className="flex flex-col gap-2 rounded-xl bg-white p-4 ring-1 ring-gray-100 shadow hover:shadow-md">
      ...
      {item.descricao && (
        <p className="line-clamp-2 text-sm text-gray-600">{item.descricao}</p>
      )}
